...
To move beyond notebook prototyping, all reusable code should go into the `scrapping/` folder package. To use that package inside your project, install the project's module in editable mode, so you can edit files in the `scrapping` folder and use the modules inside your notebooks :
...
```python
from scrapping.utils.paths import data_dir
data_dir()
```
